...
def get_bits(byte, start, end=None):
    # skip 2 chars which are 0b
    start += 2
    if end:
        #
        end += 3
        result = bin(byte)[start:end]
    else:
        result = bin(byte)[start]
    return result
...
def cli():
    print(f'1) Header')
    print(f'2) Logical Screen Descriptor')
    print(f'3) Global Color Table (optional)')
    print(f'10) Application Extension')
    print(f'for each img:')
    print(f'7) Graphic Control Extension')
    print(f'4) Image Descriptor')
    print(f'5) Local Color Table')
    print(f'6) Table Based Image Data (optional)')
    print(f'stop here')
    print(f'8) Comment Extension')
    print(f'9) Plain Text Extension')
    print(f'12) Trailer')
